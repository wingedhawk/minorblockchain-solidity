...
export default class Paymentmb extends Component {
  ...
  async loadBlockchainData() {
    ...
    setBufferedAmount(e) {
      this.setState({
        bufferedamount: e.target.value
      })
      console.log(e.target.value + " buffered somthing");
  }
                ...
                <div>
                    <div className="row">
                        <div className="col-md-6">
                            <button onClick={this.addFunds.bind(this)} className="btn btn-primary">Add buffered to channel.value</button>
                        </div>
                        <div className="col-md-6">
                            <button onClick={this.depositToChannel.bind(this)} className="btn btn-primary">Deposit to channel.balance</button>
                        </div>                            
                    </div>
                ...
                <div>
                    <div class="row">
                        <div class="col-md-4 offset-md-4">
                            <svg onClick={this.addPicture.bind(this)} width="5em" height="5em" viewBox="0 0 16 16" className="bi bi-camera2" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="form-group row">
                        <label for="claim" className="col-sm-3 col-form-label">Claim amount:</label>
                        <div class="col-sm-9">
                            <input className="form-control" id="claim-amount" type="input"></input>
                        </div>
                    </div>
                    <div className="form-group row">
                        <label for="secret" className="col-sm-3 col-form-label">Secret Message:</label>
                        <div class="col-sm-9">
                            <input className="form-control" type="input" id="secret" onChange={this.setMessageSecret.bind(this)}></input>
                        </div>
                    </div>
                    <div className="row">
                        <div class="col-sm-6">
...
//
    async claim() {
        ...
        if(await this.verify(this.state.secret)){
            await MyEscrowContract.methods.channelClaim(
                idchannel,
                amount, // Actual ammount
                amount, // planned ammount
                this.state.secret,
        ...
        document.getElementById('hash-comparison').innerHTML += 'Alice claimed ' + amount + '<br>';
        this.displayContributionChannelBalances();
    }

    async withdraw() {
        let MyEscrowContract = await this.getContractCC();
        let amount = await this.checkBalanceMPE(Alice).then(result => {return result});

        await MyEscrowContract.methods.withdraw(amount, this.state.account).send({from:Bob});
        document.getElementById('hash-comparison').innerHTML += 'Withdrawed: ' + amount + '<br>';
    ...
    async addFunds(){
        ...
        var idchannel = document.getElementById('paymentchannelid').value;
        var amount = parseInt(document.getElementById('buffered').value);
        ...
        await MyEscrowContract.methods.channelAddFunds(idchannel,amount).send({from:Bob, gas: 6721975, gasPrice: 2000000}).then()
            .catch(error => console.log(error + " send"));

        await this.displayContributionChannelBalances();
        document.getElementById('buffered').value = 0;
    }

    async addPicture() {
        let MyEscrowContract = await this.getContractCC();
        document.getElementById('buffered').value = (parseInt(document.getElementById('buffered').value)+1);
    }
